...
pub enum Animation { None, Flicker, Pulse }
...
#[derive(Debug, Clone, PartialEq, Default)]
pub enum HAnchor { Left, #[default] Right, Center }

#[derive(Debug, Clone, PartialEq, Default)]
pub enum VAnchor { Top, #[default] Bottom, Center }

impl Anchor {
    // Parse: "left,top" or "center,bottom,10,20"
    fn parse(s: &str) -> Self {
        let p: Vec<&str> = s.split(',').map(str::trim).collect();
        let h = p.get(0).map(|v| match v.to_lowercase().as_str() {
            "left" => HAnchor::Left, "center" => HAnchor::Center, _ => HAnchor::Right,
        }).unwrap_or_default();
        let v = p.get(1).map(|v| match v.to_lowercase().as_str() {
            "top" => VAnchor::Top, "center" => VAnchor::Center, _ => VAnchor::Bottom,
        }).unwrap_or_default();
        let margin_h = p.get(2).and_then(|v| v.parse().ok()).unwrap_or(10);
        let margin_v = p.get(3).and_then(|v| v.parse().ok()).unwrap_or(margin_h);
        Self { h, v, margin_h, margin_v }
    }
}
...
impl AppConfig {
    pub fn load() -> Self {
        ...
        let Ok(content) = fs::read_to_string(&config_path) else { return config };
        ...
        for line in content.lines() {
            ...
            match key {
                ...
                "text_color" => if let Some(c) = config.resolve_color(value) { config.text_color = c },
                "bg_color" => if let Some(c) = config.resolve_color(value) { config.bg_color = c },
                "position" => config.anchor = Anchor::parse(value),
                "signal" => if let Some(s) = config.parse_signal(value) { config.signals.push(s) },
                _ => {}
            }
        }
        
        // Sort by threshold (lower = higher priority)
        config.signals.sort_by(|a, b| a.threshold.partial_cmp(&b.threshold).unwrap());
        config
    }
    ...
    fn parse_signal(&self, s: &str) -> Option<Signal> {
        ...
        Some(Signal {
            ...
            animation: match p[4] { "flicker" => Animation::Flicker, "pulse" => Animation::Pulse, _ => Animation::None },
    ...
    pub fn find_signal(&self, pct: f64, state: &str) -> Option<&Signal> {
        self.signals.iter().find(|s| {
            ...
            // For discharging: trigger when battery drops to/below threshold
            // For charging: trigger when battery rises to/above threshold
            match s.state_filter.as_str() {
                "charging" => pct >= s.threshold,
                _ => pct <= s.threshold,
            }
        })
    }
